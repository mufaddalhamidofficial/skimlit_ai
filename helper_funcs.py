...
def load_and_prep_image(filename: str, img_shape=224, scale=True):
    """
...
def make_confusion_matrix(
    ...
    ax.xticks(rotation=70, fontsize=text_size)
    ax.yticks(fontsize=text_size)
...
def pred_and_plot(model: tf.keras.Model, filename: str, class_names: list):
    """
...
def create_checkpoint_callback(
    ...
    Files are saved with the filepath:
...
def calculate_results(y_true: np.array, y_pred: np.array):
    """
